...
public class FloatingStringTable {
	...
	public static Float[] colWidthsWeight = new Float[]{1f, 1f, 1f, 0.6f, 0.5f, 1.2f, 1f, 0.8f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f}; 
	...
	public void draw(){
		...
		for(int i = 0; i < cosVals.size(); i++){
			p.fill(250);
			p.noStroke();
			p.rect(x+2, y + (rowHeight * (2*i+1.5f)), (width)/header.size(), rowHeight);
			p.fill(20);
		...
		p.strokeWeight(1);
		float colWidth = width/((float)header.size()-1);
...
//		}
	...
	public void setCosVals(ArrayList<Double> vals){
		cosVals = new ArrayList<Double>(vals);
	}
